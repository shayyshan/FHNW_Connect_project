...
def init_db():
    if settings.RECREATE_DB:
        # Recreate the schema and seed with the same dummy data
    ...
    try:
        ...
        if db.query(User).count() == 0:
            ...
            # --- Required Users ---
            u1 = User(email="shannon.polak@students.fhnw.ch", username="shannonpolak", password_hash=hashed_pw)
            u2 = User(email="katrin.schuette@students.fhnw.ch", username="katrinschuette", password_hash=hashed_pw)
            u3 = User(email="alondramaria.diaznavarro@students.fhnw.ch", username="alondradiaz", password_hash=hashed_pw)
            u4 = User(email="jorge.mera@students.fhnw.ch", username="jorgemera", password_hash=hashed_pw)
